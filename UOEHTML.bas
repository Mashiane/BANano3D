...
Sub AddSpan(text As String, spanClass As String, spanStyle As Map) As UOEHTML
	Dim span As UOEHTML
	span.Initialize("").SetTag("span").AddClass(spanClass).AddContent(text)
	If spanStyle <> Null Then
		For Each k As String In spanStyle.Keys
			Dim v As String = spanStyle.Get(k)
			span.SetStyle(k,v) 
		Next
	End If
	AddContent(span.HTML)
	Return Me
End Sub

Sub AddTextArea(tid As String, text As String, txtClass As String, txtStyle As Map) As UOEHTML
	Dim span As UOEHTML
	span.Initialize(tid).SetTag("textarea").AddClass(txtClass).AddContent(text)
	If txtStyle <> Null Then
		For Each k As String In txtStyle.Keys
			Dim v As String = txtStyle.Get(k)
			span.SetStyle(k,v) 
		Next
	End If
	AddContent(span.HTML)
